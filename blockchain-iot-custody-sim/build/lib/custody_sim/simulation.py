...
class SystemSimulation:
        ...
        for i in range(len(route) - 1):
            ...
            if is_adv:
                ...
                if attack_type == "replay_attack":
                    # Replay a previously confirmed event if available
                    if self._recent_confirmed:
                        idx = self.adversary.rng.integers(0, len(self._recent_confirmed))
                        event = self.adversary.replay_event(self._recent_confirmed[idx])
                    else:
                        # No confirmed events yet — fall back to payload modification
                        event = self.adversary.modify_payload(legit_event)
                else:
                    event = self.adversary.generate_attack(legit_event, priv_key, attack_type)
            else:
                event = legit_event

            # Simulate communication latency (device → mempool)
            comm_latency = self.topology.get_communication_latency()
            yield self.env.timeout(comm_latency)
    ...
    def _process_event(self, event, legit_event=None):
        """Verify event and record metrics for all three blockchain configs."""
        t_mempool = self.env.now  # arrival at mempool

        # Confirmation time = mempool arrival + uniform wait for next block
        t_eth  = t_mempool + self.topology.rng.uniform(0, self.block_time_eth)
        t_l2   = t_mempool + self.topology.rng.uniform(0, self.block_time_l2)
        t_fast = t_mempool + self.topology.rng.uniform(0, self.block_time_fast)

        # For latency calculation: latency = confirm_time - device_create_time
        t_created = event.timestamp

        # --- Replay attack edge-case model ---
        # The PRD specifies that 2/1000 replays slip through when:
        # (a) the replayed event still sits within the skew window AND
        # (b) it arrived before the original event was confirmed on-chain.
        # We model it deterministically: a replay is detected UNLESS the replayed
        # event's timestamp is within the skew window AND the sequence number has
        # not yet been advanced because this is the first confirmation for that pair.
        #
        # We run verification with the Ethereum block time as the canonical clock.
