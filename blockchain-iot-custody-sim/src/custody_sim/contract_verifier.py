...
class AssetLedger:
    def __init__(self):
        ...
        self.last_sequence = {} 
        self.events = []
        # Composite hashes of confirmed/recorded events (replay detection)
    ...
    def update(self, event: CustodyEvent):
        ...
        self.seen_hashes.add(event.composite_hash)
...
class CustodyVerifier:
    ...
    def verify(self, event: CustodyEvent, current_block_timestamp: float) -> VerificationResult:
        ...
        # Paper tables model only on-chain contract costs; device registry checks are off-chain.
        gas_naive += 0
        gas_optimized += 0
        ...
        # Step 5: Replay (duplicate) detection
        if event.composite_hash in self.ledger.seen_hashes:
            return VerificationResult(False, "replay_detected", gas_naive, gas_optimized, (time.time()-start_time)*1000)
        ...
        # Step 7: Business rules (verifyEvent)
        gas_naive += 67800
        gas_optimized += 58300
